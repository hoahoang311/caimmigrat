...
export const database = {
  ...
  async createInquiry(inquiry: Inquiry) {
    if (!isSupabaseConfigured()) {
      console.warn(
        "Supabase not configured, inquiry not saved:",
        inquiry.email
      );
  ...
  async subscribeToNewsletter(email: string) {
    if (!isSupabaseConfigured()) {
      console.warn(
        "Supabase not configured, newsletter subscription not saved:",
        email
      );
  ...
  async getNewsletterSubscribers() {
    if (!isSupabaseConfigured()) {
      console.warn(
        "Supabase not configured, returning empty newsletter subscribers"
      );
      return [];
    }
  ...
  async createConsultationBooking(booking: ConsultationBooking) {
    if (!isSupabaseConfigured()) {
      console.warn(
        "Supabase not configured, consultation booking not saved:",
        booking.email
      );
  ...
  async getConsultationBookings() {
    if (!isSupabaseConfigured()) {
      console.warn(
        "Supabase not configured, returning empty consultation bookings"
      );
      return [];
    }
  ...
  async updateBookingStatus(
    id: string,
    status: ConsultationBooking["booking_status"]
  ) {
    const { data, error } = await supabase
      .from("consultation_bookings")
